import { CursorType, MIN_SHAPE_SIZE } from '@/constants';
import { Coordinates2D, Element, ResizeHandle, Shape, Tool } from '@/interfaces';
import { getCursorStyle } from './interactions.utils';
...
export function getHandleAtPosition(mx: number, my: number, el: Element): ResizeHandle {
	...
	if (Math.abs(localX - 0) < THRESHOLD && Math.abs(localY - (-absHalfH - 25)) < THRESHOLD) {
		return 'rotation';
	}
	if (Math.abs(localX - -absHalfW) < THRESHOLD && Math.abs(localY - -absHalfH) < THRESHOLD) {
		return 'top-left';
	}
	if (Math.abs(localX - absHalfW) < THRESHOLD && Math.abs(localY - -absHalfH) < THRESHOLD) {
		return 'top-right';
	...
	if (isYWithin && Math.abs(localX - -absHalfW) < THRESHOLD) {
		return 'left';
	}
	if (isYWithin && Math.abs(localX - absHalfW) < THRESHOLD) {
		return 'right';
	}
	if (isXWithin && Math.abs(localY - -absHalfH) < THRESHOLD) {
		return 'top';
	}
	if (isXWithin && Math.abs(localY - absHalfH) < THRESHOLD) {
		return 'bottom';
